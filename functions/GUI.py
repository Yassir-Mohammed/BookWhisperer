...
def upload_files_element(element_text="",allowed_types=["PDF"],accept_multiple_files=True):
    """
    ...
    with st.container():
        st.header("🗂 Files Commit",divider="green")
...
def upload_files_element_old(element_text="Upload PDF documents",allowed_types=["pdf"],accept_multiple_files=True):
    """
    Upload multiple PDF files via Streamlit, validate filenames,
    and categorize them into:
      1. Invalid (bad names)
...
def provide_google_secrets_file(account_type:list[str] = None):
    ...
    with st.container():
        st.header("Google Drive authentication file", divider="orange")
        try:
